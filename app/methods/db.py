...
def getInfoByEan(ean):
    ...
    _cur_.execute(
        "SELECT ean, name, type FROM \"products\" WHERE ean = %s", (ean, ))
    ...
    externalDbResult = requests.get("http://bazil.si/api/v2/search/?code="+str(
        ean), headers={"Authorization": "Bearer 1J0i8MmV2BQWJrgcVnGU"})
    if externalDbResult.status_code == 404:
        return None
    externalJson = json.loads(externalDbResult.content)
    _cur_.execute("INSERT INTO \"products\" (ean, name) VALUES (%s, %s)", (str(
        ean), externalJson[0]["NAME"]))
    _conn_.commit()
