...
def _getProductAPI_(ean):
    returned = requests.get("http://bazil.si/api/v2/search/?code=" +
                            str(ean), headers={"Authorization": "Bearer 1J0i8MmV2BQWJrgcVnGU"})
...
def _addProductToCache_(ean):
    ...
    _cur_.execute("INSERT INTO products (ean, name) VALUES (%s, %s)",
                  (str(ean), productJson[0]["NAME"]))
    _conn_.commit()
    return productJson
